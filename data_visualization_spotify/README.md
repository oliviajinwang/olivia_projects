...
There are 8582 unique tracks recorded, each accompanied with information over a wide range. For example, this includes:
* Track ID, track name, track popularity.
* Whether the song is explicit or not.
* Artist's name, popularity, and follower count (On Spotify).
* Artist genres.
* Albumn name, release date, type and duration.
...
* For the songs with multiple genres, it firstly didn't make sense to only choose one of the listed genres. Therefore, I found all the matching genre categories to the ones listed under the song, and split the weight evenly among all the matches
* I chose to parse the string into a list by using the .split() function, then cleaned the strings by trimming spaces.
